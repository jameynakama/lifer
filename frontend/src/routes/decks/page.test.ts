...
const decks = [{ id: 1, name: 'My Warblers', audio_due: 2, image_due: 0 }]
...
describe('Decks page', () => {
      ...
      vi.fn().mockImplementation((url: string, opts?: RequestInit) => {
        if (opts?.method === 'POST') {
          ...
          return Promise.resolve({
            ok: true,
            json: () => Promise.resolve({ id: 2, name: 'New Deck', audio_due: 0, image_due: 0 }),
          })
        }
        return Promise.resolve({
          ok: true,
  ...
  it('shows description below deck name when provided', async () => {
    const decksWithDesc = [
      {
        id: 1,
        name: 'Woodpeckers',
        description: 'Confusing laughing calls',
        audio_due: 0,
        image_due: 0,
      },
    ]
    vi.stubGlobal(
      ...
      vi.fn().mockResolvedValue({
        ok: true,
        json: () => Promise.resolve({ decks: decksWithDesc, next_due_at: null }),
      }),
    )
    renderWithClient(DecksPage)
    await vi.waitFor(() => {
  ...
  it('sends description to API when creating a deck', async () => {
    ...
    await fireEvent.click(screen.getByRole('button', { name: /create/i }))
    await vi.waitFor(() => {
      expect((body as Record<string, unknown>).description).toBe('A description')
    })
  })
  ...
  it('hides due badges in practice mode', async () => {
    ...
    vi.stubGlobal(
      ...
      vi.fn().mockResolvedValue({
        ok: true,
        json: () => Promise.resolve({ decks: deckWithDue, next_due_at: null }),
      }),
    )
    renderWithClient(DecksPage)
